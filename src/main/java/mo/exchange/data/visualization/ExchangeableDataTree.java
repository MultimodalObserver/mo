...
public class ExchangeableDataTree extends JCheckBoxTree
{
	private DefaultMutableTreeNode root;
	private DefaultTreeModel model;
	private ProjectOrganization organization;
	private String filter = "PARTICIPANTS_AND_STAGES";
	private List<ExchangeableData> data = new ArrayList();
                	...
                	if (pathelEments.length == 2) {
                		for (Object element : pathelEments) {
                			...
                			if (nodeContent instanceof ParticipantTreeNode) {
                				...
                				dataElement = new ExchangeableData();
                				...
                				for (int i = 0; i < n ; i++) {
                					...
                					sNode = (StageTreeNode) childNode.getUserObject();
                					stage = sNode.getStage();
                					stages.add(stage);
                				}
                				dataElement.stages = stages;
		...
		for (Participant participant : organization.getParticipants()) {
			...
			for (StageModule stage : organization.getStages() ) {
				...
				if (Files.exists(stageDir.toPath())) {
					stageNode = new DefaultMutableTreeNode(new StageTreeNode(stage));
	...
	public List<ExchangeableData> getExchangeableData() {
		return data;
	}
